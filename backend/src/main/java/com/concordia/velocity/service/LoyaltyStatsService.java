 ...
 * LoyaltyStatsService:
 * ---------------------
 * This service computes ALL historical metrics required by the Loyalty Tier System.
 *
...
public class LoyaltyStatsService {
    ...
    private int countTripsLastYear(List<Trip> trips) { // Counts trips that occurred within the last 12 months, will be used by Bronze, Silver, Gold
    ...
    private int countMissedReservations(List<Trip> trips) { // Counts missed or expired reservations, for Bronze
        int count = 0;
        for (Trip trip : trips) {
    ...
    private int countSuccessfulClaims(List<Trip> trips) { // Counts completed trips, for Silver
        int count = 0;
        for (Trip trip : trips) {
    ...
    private List<Integer> computeTripsPerWeek(List<Trip> trips) {
        ...
        for (int i = 0; i < 12; i++) weeks.add(0);
    ...
    /**
     * Computes week difference between current week and trip week because we need to know how many weeks ago a trip happened. For gold tier,
     * it requires that the user has 5 trips every week for the last 12 weeks. It handles the case where the trip was in this current year, in
     ...
     * For example, storing tripsPerWeek[0] = number of trips this week, tripsPerWeek[1] = number of trips last week, etc.
     */

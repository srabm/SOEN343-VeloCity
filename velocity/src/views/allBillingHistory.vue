...
async function loadBillingHistory() {
  loading.value = true;
  try {
    // Fetch ALL bills from the top-level bills collection
    const billsRef = collection(db, "bills");
    const billsSnap = await getDocs(billsRef);
    
    // Extract bill data and enrich with rider and trip information
    const extractedBills = [];
    
    for (const billDoc of billsSnap.docs) {
      const billData = billDoc.data();
      
      // Fetch rider email from riders collection
      let riderEmail = "—";
      if (billData.riderId) {
        try {
          const riderRef = doc(db, "riders", billData.riderId);
          const riderSnap = await getDoc(riderRef);
          if (riderSnap.exists()) {
            riderEmail = riderSnap.data().email || billData.riderId;
          }
        } catch (error) {
          console.error("Error fetching rider:", error);
          riderEmail = billData.riderId;
        }
      }
      ...
      if (billData.tripId) {
        try {
          const tripRef = doc(db, "trips", billData.tripId);
      ...
      extractedBills.push({
        ...
        paymentMethodLastFour: billData.paymentMethodLastFour,
        riderId: billData.riderId,
        riderEmail: riderEmail,
        tripId: billData.tripId,
        
        // Trip-related data from trips collection
        bikeId: bikeId,
...
const filteredBills = computed(() => {
  return bills.value.filter((bill) => {
    ...
    // Rider Email filter
    if (filters.value.riderEmail) {
      const searchTerm = filters.value.riderEmail.toLowerCase();
      if (!bill.riderEmail?.toLowerCase().includes(searchTerm)) {
        return false;
      }
    }
